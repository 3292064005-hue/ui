...
import { Camera, Maximize2, Download, XCircle } from 'lucide-react';
...
export default function CameraFeed() {
  ...
  const snapshot = useCallback(() => {
    ...
    a.download = `camera_${Date.now()}.png`;
    a.click();
  }, []);

  const containerClass = fullscreen
    ? 'fixed inset-0 z-[90] bg-black/95 flex flex-col items-center justify-center p-8'
    : 'glass-panel p-2 flex flex-col pointer-events-auto shadow-2xl transition-all duration-300';
  ...
  return (
    <div className={containerClass}>
      ...
      <div className="flex items-center justify-between w-full mb-1.5 px-1">
        <h3 className="text-[10px] text-clinical-cyan font-bold tracking-widest uppercase flex items-center">
      ...
      {/* Video Frame */}
      <div className={`relative ${videoClass} bg-black/50 rounded-lg overflow-hidden border border-white/5`}>
        {frame ? (
          <img ref={imgRef} src={frame} alt="Camera" className="w-full h-full object-cover" crossOrigin="anonymous" />
        ) : (
          <div className="flex w-full h-full items-center justify-center text-gray-500 font-mono text-xs">
            <Camera className="w-5 h-5 mr-2 opacity-30" /> 等待 OpenCV...
          </div>
        )}
      </div>
    </div>
