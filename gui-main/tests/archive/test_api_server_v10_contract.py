...
class _StubAdapter:
    ...
    def current_command_policy_snapshot(self) -> dict: return {'session_id': 'S1', 'execution_state': 'PAUSED_HOLD', 'decision_count': 4}
...
def test_api_server_v10_command_policy_snapshot(monkeypatch: pytest.MonkeyPatch):
    with _client(monkeypatch) as client:
        response = client.get('/api/v1/sessions/current/command-policy-snapshot')
        assert response.status_code == 200
        body = response.json()
        assert body['execution_state'] == 'PAUSED_HOLD'
        assert body['decision_count'] == 4
