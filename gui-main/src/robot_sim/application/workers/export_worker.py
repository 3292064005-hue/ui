...
class ExportWorker(BaseWorker):
    """Qt worker wrapper for long-running export operations."""

    def __init__(self, func, *args, **kwargs):
        """Create an export worker.

        Args:
            func: Callable that performs the export operation.
            *args: Positional arguments forwarded to ``func``.
            **kwargs: Keyword arguments forwarded to ``func``.
        """
        super().__init__(task_kind='export')
        self._func = func
        self._args = args
        self._kwargs = kwargs
    ...
    @Slot()
    def run(self):
        """Execute the export callable and emit terminal worker events."""
        self.emit_started()
        try:
            if self.is_cancelled():
                self.emit_cancelled(stop_reason='cancelled')
                return
            result = self._invoke_with_control()
            if self.is_cancelled():
                self.emit_cancelled(stop_reason='cancelled')
                return
