...
class ControlAuthorityService:
    ...
    def acquire(
        ...
        *,
        ...
        now_ts = now_ns()
        active = self._get_active_lease(now_ts)
        if active and not self._same_owner(active, actor, normalized_workspace, normalized_role):
            if preempt and self._can_preempt(active, actor, normalized_role):
                ...
                self._record_event("lease_preempted", actor, normalized_workspace, normalized_role, session_id, detail)
            else:
                detail = self._conflict_detail(active, actor, normalized_workspace, normalized_role)
                self._last_conflict_reason = detail
                self._record_event("lease_conflict", actor, normalized_workspace, normalized_role, session_id, detail)
                return {
                    "ok": False,
        ...
        lease_id = active.lease_id if active and self._same_owner(active, actor, normalized_workspace, normalized_role) else self._make_lease_id(actor, normalized_workspace, normalized_role, session_id)
        self._active_lease = ControlLease(
            ...
            acquired_ts_ns=active.acquired_ts_ns if active and self._same_owner(active, actor, normalized_workspace, normalized_role) else now_ts,
            refreshed_ts_ns=now_ts,
            expires_ts_ns=now_ts + ttl * 1_000_000_000,
            source=source,
        )
        self._last_conflict_reason = ""
        event_name = "lease_acquired" if not active or not self._same_owner(active, actor, normalized_workspace, normalized_role) else "lease_refreshed"
        self._record_event(event_name, actor, normalized_workspace, normalized_role, session_id, self._active_lease.lease_id)
        return {
            "ok": True,
            "summary_state": "ready",
    ...
    def renew(self, *, lease_id: str, actor_id: str | None = None, ttl_s: int | None = None) -> dict[str, Any]:
        ...
        self._record_event("lease_renewed", active.actor_id, active.workspace, active.role, active.session_id, active.lease_id)
        return {
            "ok": True,
            "summary_state": "ready",
    ...
    def release(self, *, lease_id: str | None = None, actor_id: str | None = None, reason: str = "") -> dict[str, Any]:
        ...
        self._record_event("lease_released", active.actor_id, active.workspace, active.role, active.session_id, reason or active.lease_id)
    ...
    def bind_session(self, session_id: str) -> None:
        ...
        self._record_event("lease_bound_session", self._active_lease.actor_id, self._active_lease.workspace, self._active_lease.role, session_id, self._active_lease.lease_id)
    ...
    def guard_command(
        ...
        *,
        ...
        deployment_profile = self._normalize_profile(str(context.get("profile") or context.get("deployment_profile") or normalized_payload.get("profile") or DEFAULT_PROFILE))
    ...
    def _get_active_lease(self, now_ts_ns: int | None = None) -> ControlLease | None:
        ...
        if active.expires_ts_ns <= current_ts:
            self._record_event("lease_expired", active.actor_id, active.workspace, active.role, active.session_id, active.lease_id)
            self._active_lease = None
            return None
        return active
