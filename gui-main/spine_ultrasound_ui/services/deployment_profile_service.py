...
_PROFILE_ORDER = ("dev", "research", "clinical", "review")
...
class DeploymentProfileService:
    ...
    def resolve(self, config: RuntimeConfig | None = None) -> DeploymentProfile:
        ...
        if requested == "clinical":
            return DeploymentProfile("clinical", True, True, True, False, True, ("operator", "service"), "Clinical execution profile with strict control ownership, token-gated writes and sealed session evidence.", log_granularity="audit", seal_strength="strict", provenance_strength="strict")
        if requested == "research":
            return DeploymentProfile("research", True, False, True, False, False, ("operator", "researcher", "service"), "Research execution profile with writable runtime and evidence capture enabled.", log_granularity="verbose", seal_strength="strong", provenance_strength="strong")
        if requested == "review":
            return DeploymentProfile("review", False, False, True, True, False, tuple(), "Read-only review profile for replay, QA and exported evidence inspection.", log_granularity="audit", seal_strength="strict", provenance_strength="strict")
        return DeploymentProfile("dev", True, False, False, False, False, ("operator", "researcher", "qa", "service"), "Development profile optimized for local iteration and mock/runtime debugging.", log_granularity="debug", seal_strength="relaxed", provenance_strength="standard")
